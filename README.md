...
`yunzhi` 是一个终端内对话式智能体工具，交互模式默认启用 ratatui/crossterm 全屏 TUI：上方是可滚动输出区，下方是固定多行输入框，中间状态栏展示本轮/会话 token、耗时和费用估算。
...
- 支持 `read_file`、`write_file`、`edit_file`、`append_file`、`create_dir`、`copy_path`、`move_path`、`delete_path`、`file_info`、`bash`、`execute_code`、`run_program`、`glob_search`、`grep_search`、`code_index`、`git_manager`、`test_loop`、`list_dir`、`ask_user`、`choose_option`、`list_models`、`list_skills`、`read_skill`、`add_skill`、`list_mcp_servers`、`add_mcp_server`、`call_mcp_tool`、`mcp_resource`、`mcp_prompt`、`manage_todos`、`system_control`、`call_model`。
...
- 支持数据与记忆操作：`database_manager` 调用 sqlite/psql/mysql/redis-cli 查询或管理数据库，`long_memory` 读取、追加、替换、清空项目长期记忆 `.yunzhi/memory.md`，也支持按条目 `add`、`list`、`search`、`delete` 管理标签化记忆。
- 主控模型可通过 `--model` 或交互模式 `/model <模型名>` 切换；也可以通过 `list_models` 读取云智 API 可用模型列表，并通过 `call_model` 工具调用其他模型完成子任务或交叉检查。
- 支持本地 Skill：启动时索引 `.yunzhi/skills` 与 `~/.yunzhi/skills`，模型可用 `list_skills` 查看技能，用 `read_skill` 读取完整 Markdown 指令后执行，也可用 `add_skill` 添加项目级或用户级 Skill。
- 支持 MCP stdio client：读取 `.yunzhi/mcp.json` 与 `~/.yunzhi/mcp.json`，模型可用 `list_mcp_servers` 查看 server，用 `add_mcp_server` 新增配置，用 `call_mcp_tool` 发起 `tools/call`，用 `mcp_resource` 调用 `resources/list`/`resources/read`，用 `mcp_prompt` 调用 `prompts/list`/`prompts/get`。
...
- 支持全屏交互体验：固定输入框、可滚动输出区、状态栏、实时工具调用可视化、权限确认弹窗、diff 预览、多行输入、历史记录、`/` 命令补全和 `@` 文件补全。
...
可以用命令直接添加、列出和读取 Skill：

```bash
yunzhi skill add code/review --description "Rust 代码审查流程" --file ./review-skill.md
yunzhi skill add writing/ppt --description "PPT 制作流程" --content "按目标、受众、结构和版式输出。" --scope user
yunzhi skill list
...
也可以用命令新增和查看 MCP server 配置：

```bash
yunzhi mcp add filesystem node --arg ./mcp-filesystem-server.js --env ROOT=/workspace
yunzhi mcp add yunzhi yunzhi --arg mcp-server --scope user
yunzhi mcp list
```

`call_mcp_tool` 会按 MCP stdio JSON-RPC 初始化 server，并调用 `tools/call`。由于会启动外部进程，默认需要权限确认。`mcp_resource` 支持列出和读取资源，`mcp_prompt` 支持列出和获取提示模板，用于对齐 MCP resources 与 prompts 能力。

yunzhi 也可以作为 MCP server 暴露给其他工具。MCP client 配置示例：
...
server 模式通过 stdio 通信，暴露 yunzhi 内置工具列表、当前 workspace 资源和 `code_review`、`test_plan` 两个 prompt 模板。
...
交互模式使用全屏 TUI 渲染，模型文本流、工具调用参数、执行完成状态、权限确认和 diff 预览都会进入同一个事件通道；`print` 非交互模式保留 stdout 流式输出，便于脚本和管道使用。
